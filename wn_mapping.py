...
def wn_sense_mapping(wn31_id, df_wn30, df_wn31): 
    id_ = str(wn31_id.split('-')[0])
    df_wn31_id = df_wn31[df_wn31['synset_offset'] == id_]
    sense_keys31 = df_wn31_id.sense_key.to_list()
    wn_ids30 = []
    senses = []
    for sense in sense_keys31: 
        df_wn30_id = df_wn30[df_wn30['sense_key'] == sense]
        if len(df_wn30_id) > 0: 
            wn30_id = df_wn30_id.synset_offset.to_list()[0]+'-'+str(wn31_id.split('-')[1])
            senses.append(sense) 
            # if len(wn30_id) > 1: 
            #     print(wn30_id)
...
wikidata_wn = os.path.join('database', 'query_wikidata_identificador_de_synset_de_WordNet_3_1.tsv')
df_wikidata_wn = pd.read_csv(wikidata_wn, sep='\t', dtype='str')
