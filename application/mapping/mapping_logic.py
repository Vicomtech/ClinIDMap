...
def icd10_in_umls(umls_cuis, icd10_type): 
    ...
    for cui in umls_cuis: 
        q_dic = get_umls_query(cui, icd10_type)
        # print(q_dic)
...
def map_source_umls(source_id): 
    ...
    q_dic = get_umls_query(source_id, 'SNOMEDCT_US') 
    print('QUERY', q_dic)
    
    umls_result = query_search(q_dic, constants.UMLS_EXT)
    # print('SNOMED in UMLS', umls_result)
    print(umls_result)
...
def cui2sty(mapped_cuis): 
    ...
    for cui in mapped_cuis: 
        ...
        tuis = result2list_(sty_result, 'TUI')
        defs = result2list_(sty_result, 'STY')
